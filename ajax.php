...
function save_new_asset($n_asset_tag, $n_date_purchase, $n_asset_name, $n_serial_no, $n_description, $type_of_asset, $n_sbu, $n_price, $n_installed_os, $n_ms_office, $n_remarks, $n_type, $n_status){   
    ...
    $insert_new_asset=mysqli_prepare($conn,"CALL new_asset(?,?,?,?,?,?,?,?,?,?,?,?,?)"); mysqli_stmt_bind_param($insert_new_asset,"sssssssssssss",$n_asset_tag,$n_asset_name,$n_description,$n_serial_no,$n_price,$n_sbu,$n_date_purchase,$n_installed_os,$n_ms_office,$n_remarks,$n_type,$n_status,$qty);
...
//    $update_asset_details = mysqli_prepare($conn,"CALL new_asset(?,?,?,?,?,?,?,?,?,?,?,?)"); mysqli_stmt_bind_param($update_asset_details,"ssssssssssss",$asset_tag, $date_purchase, $asset_name, $serial_no, $description, $sbu, $price, $installed_os, $ms_office, $remarks, $type, $status);
//    $update_asset_details->execute();
//    
//    echo $update_asset_details;
...
function get_employee($employee_details){    
    ...
    $query = "SELECT * FROM tbl_employees WHERE id = '$search_id'";
    ...
    else
    {
        ...
        if(mysqli_query($conn, $query))
        {
            ...
            $query = "UPDATE tbl_assets ass 
                      LEFT JOIN tbl_records rec ON ass.id = rec.asset_id
...
function acc_get_employee($emp_details){
    ...
    {     
        ...
        $query = "SELECT rec.id, emp.id, rec.date_created, rec.emp_fname, rec.emp_lname, sbu.sbu_name, dept.dept_name, rec.emp_position, rec.asset_tag, rec.asset_name, rec.asset_type_of, rec.asset_descriptions, rec.asset_remarks, rec.record_status, rec.acc_status
            FROM tbl_employees emp
            INNER JOIN tbl_records rec ON emp.id = rec.emp_id
            INNER JOIN tbl_sbu sbu ON sbu.id = emp.sbu_id
            INNER JOIN tbl_dept dept ON dept.id = emp.dept_id
            WHERE rec.record_status = 0
...
function MainDevices($selected_sbu){
    ...
    $query = "SELECT COUNT(CASE WHEN ass.`status` IN('Available','In use') then 1 ELSE NULL END) AS Working, COUNT(CASE WHEN ass.`status` = 'Missing' then 1 ELSE NULL END) AS Missing, COUNT(CASE WHEN ass.`status` = 'Disposed' then 1 ELSE NULL END) AS Disposed, COUNT(CASE WHEN ass.`status` = 'Defective' then 1 ELSE NULL END) AS Defective, COUNT(CASE WHEN ass.`status` = 'Spare of Device Available' then 1 ELSE NULL END) AS Spare, COUNT(ass.`status`) AS Total
    FROM tbl_assets ass
...
function dashboard_table_assets($selected_sbu){
    ...
    {
        case 'PNKC':
            $query = "SELECT DISTINCT(items.itemname), COUNT(CASE WHEN assets.`status` IN('Available','In use') then 1 ELSE NULL END) AS Working, COUNT(CASE WHEN assets.`status` = 'Missing' then 1 ELSE NULL END) AS Missing, COUNT(CASE WHEN assets.`status` = 'Disposed' then 1 ELSE NULL END) AS Disposed, COUNT(CASE WHEN assets.`status` = 'Defective' then 1 ELSE NULL END) AS Defective, COUNT(CASE WHEN assets.`status` = 'Spare of Device Available' then 1 ELSE NULL END) AS Spare, COUNT(assets.`status`) AS Total
                    FROM tbl_item items 
                    LEFT JOIN tbl_assets assets ON items.id = assets.type_of AND assets.type_of = items.id
                    WHERE items.type_of_item = 0
                    AND assets.sbu = 'PNKC'
                    GROUP BY itemname
                    ";
            $result = mysqli_query($conn, $query);
        ...
        break;
        case 'EXERGY':
            $query = "SELECT items.itemname, COUNT(CASE WHEN assets.`status` IN('Available','In use') then 1 ELSE NULL END) AS Working, COUNT(CASE WHEN assets.`status` = 'Missing' then 1 ELSE NULL END) AS Missing, COUNT(CASE WHEN assets.`status` = 'Disposed' then 1 ELSE NULL END) AS Disposed, COUNT(CASE WHEN assets.`status` = 'Defective' then 1 ELSE NULL END) AS Defective, COUNT(CASE WHEN assets.`status` = 'Spare of Device Available' then 1 ELSE NULL END) AS Spare, COUNT(assets.`status`) AS Total
                    FROM tbl_item items 
                    LEFT JOIN tbl_assets assets ON items.id = assets.type_of AND assets.type_of = items.id
                    WHERE items.type_of_item = 0
                    AND assets.sbu = 'EXERGY'
                    GROUP BY itemname
                    ";
            $result = mysqli_query($conn, $query);
            echo $conn->error;
...
function getdatatochart($selected_sbu){
    ...
    $query = "SELECT itemname,count(*) FROM tbl_item item 
                LEFT JOIN tbl_assets assets ON item.id = assets.type_of
                where item.type_of_item = 0
                group by itemname";
    $result = mysqli_query($conn, $query);
    while($row = mysqli_fetch_array($result)){
...
function getdatatochartP(){
    ...
    $query = "SELECT itemname,count(*) FROM tbl_item item 
                LEFT JOIN tbl_assets asset ON item.id = asset.type_of
                where item.type_of_item = 1
                group by itemname";
    $result = mysqli_query($conn, $query);
    while($row = mysqli_fetch_array($result)){
...
function userlists(){
    ...
    $query = "SELECT emp.id, emp.employee_id, CONCAT(emp.last_name,' ', emp.first_name) AS EmpName, emp.email, emp.user_name, sbu.sbu_name, dept.dept_name, emp.position, us.user_legend
            FROM tbl_employees emp
            INNER JOIN tbl_sbu sbu ON emp.sbu_id = sbu.id
            INNER JOIN tbl_dept dept ON emp.dept_id = dept.id
            INNER JOIN tbl_users us ON emp.user_type = us.id
...
function addusers($lastname, $firstname, $midinitial, $username, $email, $password_1, $password_2, $user_type, $emp_sbu, $position, $department){
    ...
    $insert_new_user = mysqli_prepare($conn, "CALL new_users(?,?,?,?,?,?,?,?,?,?,?,?)");
    mysqli_stmt_bind_param($insert_new_user,"ssssssssssss", $id, $firstname, $lastname, $midinitial, $email, $username, $position, $password_1, $password_2, $emp_sbu, $department, $user_type);
