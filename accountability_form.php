...
$sbu_asset = $_SESSION['asset_SBU'];
$formid = $_SESSION['form_id'];
    $query = "SELECT rec.id, rec.date_created, rec.location, rec.record_status, rec.acc_status, emp.id, CONCAT(emp.first_name,' ',emp.last_name), emp.position, dept.dept_name, sbu.sbu_name, rec.asset_tag, rec.asset_name, item.itemname, rec.asset_descriptions, rec.asset_remarks, ass.quantity 
...
switch($sbu_asset)
{
    ...
    public function Header() {
        ...
        $this->Cell(196, 10, 'PHIL NIPPON KYOEI awdawCORPORATION', 0, 1, 'C');
    ...
    public function Footer() {
        include 'dbcon.php';
        $employeeid = $_SESSION['employee_id']; 
        $pnpfullname = strtoupper($_SESSION['pnpfname']);
        $f_issuer = strtoupper($_SESSION['issuer']);
        ...
        $this->SetFont('helvetica', '', 8);
        $this->MultiCell(90, 15, $pnpfullname, 1, 'C', 0, 0, '', '', true, 0, false, true, 15, 'B');
    ...
    $pdf->SetFont('helvetica', '', 8);
    $pdf->Cell(40, 0, $date_created, 1, 0, 'C', 0, '', 0, false, 'T', 'M');
    $pdf->SetFont('helvetica', 'B', 8);
    $pdf->Cell(40, 0, 'DEPARTMENT:', 1, 0, 'C', 0, '', 0, false, 'T', 'M');
    $pdf->SetFont('helvetica', '', 8);
    $pdf->Cell(70, 0, $dept_name, 1, 1, 'C', 0, '', 0, false, 'T', 'M');
    $pdf->Ln(1);
    $pdf->SetFont('helvetica', 'B', 8);
    ...
    //        </table>';
                ...
                $pdf->MultiCell(35, 10, $asset_type_of, 1, 'C', 0, 0, '', '', true);
                $pdf->MultiCell(65, 10, $asset_descriptions, 1, 'C', 0, 0, '', '', true);
                $pdf->MultiCell(15, 10, $assetQty, 1, 'C', 0, 0, '', '', true);
    ...
    public function Header() {
        ...
        // Title
        $this->Cell(180, 10, 'FORM 2023', 0, 1, 'R');
        $this->Cell(196, 10, 'EXERGY PHILIPPINES CORPORATION', 0, 1, 'C');
        $this->SetFont('helvetica', '', 10);
        $this->Cell(185, 2, 'Suite 705 Royal Plaza Twin Towers.', 0, 1, 'C');
        $this->Cell(182, 2, '648 Remedios St., Malate, Manila', 0, 1, 'C');
        $this->SetFont('helvetica', 'B', 14);
